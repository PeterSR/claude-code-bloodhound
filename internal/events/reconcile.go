...
func applyReadingsOnce(ctx context.Context, db *sql.DB, now time.Time, readings []Reading, st *Stats) error {
	...
	rows, err := tx.QueryContext(ctx, `SELECT kind, bucket, session_uuid, state FROM event_levels`)
	if err != nil {
		return err
	}
	for rows.Next() {
		var k levelKey
		var state string
		if err := rows.Scan(&k.kind, &k.bucket, &k.session, &state); err != nil {
			rows.Close()
			return err
		}
		current[k] = state
	...
	for _, r := range readings {
		...
		k := levelKey{r.Kind, r.Scope.Bucket, r.Scope.Session}
...
func upsertLevel(ctx context.Context, ex execer, k levelKey, state string, tsMS int64) error {
	_, err := ex.ExecContext(ctx, `
		INSERT INTO event_levels (kind, bucket, session_uuid, state, since_ms)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(kind, bucket, session_uuid)
		DO UPDATE SET state = excluded.state, since_ms = excluded.since_ms`,
		k.kind, k.bucket, k.session, state, tsMS)
	return err
}
